...
describe('applyLayoutPatch', () => {
  it('merges only rect fields and keeps identity of everything else', () => {
    const source = [
      { id: 'a', type: 'text', left: 0, top: 0, width: 500, height: 100, rotate: 0, content: '<p>keep</p>' },
      { id: 'b', type: 'text', left: 10, top: 20, width: 400, height: 200, rotate: 0, content: '<p>keep2</p>' },
    ];
  ...
  it('rejects a patch that drops an element id', () => {
    ...
    expect(
      applyLayoutPatch(canvas, [canvas.elements[0]] as never),
    ).toBe(false);
  });
  ...
  it('rejects resizes beyond ±20%', () => {
    const canvas = {
      ...
      viewportRatio: 0.5625,
      elements: [
        { id: 'a', type: 'text', left: 0, top: 0, width: 100, height: 100, rotate: 0 },
      ],
    };
    const applied = applyLayoutPatch(canvas, [{ id: 'a', left: 0, top: 0, width: 300, height: 100 } as never]);
    expect(applied).toBe(true);
...
describe('verifyAndRepairSlideLayout', () => {
  ...
  it('calls the layout repair route when clamping cannot clear the findings', async () => {
    ...
    vi.stubGlobal('fetch', fetchSpy);
    const content = slideContent([
      { id: 'a', type: 'text', left: 0, top: 0, width: 500, height: 100, rotate: 0, content: '<p>x</p>' },
      { id: 'b', type: 'text', left: 40, top: 10, width: 500, height: 400, rotate: 0, content: '<p>y</p>' },
    ]);
    const result = await verifyAndRepairSlideLayout(content);
