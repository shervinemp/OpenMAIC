...
// The curriculum shape is a validated contract (see blueprint.ts): scene
// count is derived from a resolved duration and enforced by validateBlueprint
// -- never accepted on model whim.
...
export interface CourseSizePresetConfig {
  ...
  maxLessons: number;
  /** Scene density used to derive the total from the duration. */
