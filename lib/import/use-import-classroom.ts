...
/** A classroom ZIP whose manifest is missing, unreadable, or incomplete. */
export class ImportClassroomError extends Error {
  readonly code: 'INVALID_MANIFEST' | 'MISSING_DATA';

  constructor(code: 'INVALID_MANIFEST' | 'MISSING_DATA', message?: string) {
    super(message || code);
 ...
 *
 * Shared by the home-page picker (`useImportClassroom`) and the full local
 * backup restore. Resolves to the new stage id once the document committed.
 * On failure the local rows and the document are rolled back before the
 * error is rethrown; server allocations are left to the pending-asset
 * collector, since the browser cannot safely delete them after a document
 * write with an ambiguous outcome.
 */
export async function importClassroomZip(
  ...
  if (!manifest.stage || !manifest.scenes || !Array.isArray(manifest.scenes)) {
    throw new ImportClassroomError('MISSING_DATA');
  }
  ...
  try {
    // Store media before publishing the document's references.
    ...
    // replaces. A classroom package may carry an older updatedAt, so it opts
    // out of the stale-write fence.
...
export function useImportClassroom(onSuccess?: (importedStageId: string) => void) {
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Only ever set by a COMMITTED import: importClassroomZip owns the
      // rollback of a failed one, so there is nothing to undo here.
      let importedStageId: string | undefined;
      try {
        const JSZip = (await import('jszip')).default;
        const zip = await JSZip.loadAsync(file);
        importedStageId = await importClassroomZip(zip, (next) => {
          setPhase(next);
          if (next === 'validating') toast.loading(t('import.validating'), { id: toastId });
          else if (next === 'writingMedia')
            toast.loading(t('import.writingMedia'), { id: toastId });
          else if (next === 'writingCourse')
