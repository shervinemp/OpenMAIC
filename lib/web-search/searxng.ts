...
async function fetchSearxngWithRetry(url: string): Promise<Response> {
  const MAX_ATTEMPTS = 3;
  const RETRY_DELAY_MS = [1000, 2000, 4000];
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    const res = await proxyFetch(url, { method: 'GET', headers: SEARXNG_HEADERS });
...
export async function searchWithSearxng(params: {
  ...
  const res = await fetchSearxngWithRetry(requestUrl);
